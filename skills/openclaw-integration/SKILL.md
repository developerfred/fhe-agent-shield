    ...
    return {
      ...
      async execute(context: SkillContext): Promise<SkillResult> {
        ...
        if (config.requirePermits?.length) {
          const hasPermits = await checkCredentialPermits(
            config.credentialVault!,
            config.requirePermits
          );
          if (!hasPermits) {
...
class FHEAgentMemoryProvider implements MemoryProvider {
  ...
  async appendContext(chunk: any): Promise<number> {
    ...
    const [encrypted] = await this.fheClient.encryptInputs([
      Encryptable.bytes(JSON.stringify(chunk))
    ]);
  ...
  async getContext(offset: number, length: number): Promise<any[]> {
    ...
    const encryptedChunks = await this.contract.getContextSlice(
      this.agentId,
      offset,
      length
    );
    ...
    const decrypted = await Promise.all(
      encryptedChunks.map(chunk => this.fheClient.decrypt(chunk))
    );
...
class FHECredentialVault {
  ...
  async getCredential(key: string): Promise<string> {
    ...
    const handle = await this.vault.getCredentialHandle(
      this.agentId,
      FHE.asEuint256(keccak256(key))
    );
  ...
  async requestCredentialAccess(key: string, permittedAddress: string): Promise<void> {
    await this.vault.grantRetrievePermission(
      this.agentId,
      FHE.asEuint256(keccak256(key)),
      permittedAddress
    );
...
describe('FHE-Protected Agent', () => {
  ...
  it('should block skill without permits', async () => {
    ...
    await expect(
      agent.execute({ prompt: 'Do something dangerous' })
    ).to.be.revertedWith('MissingRequiredPermit');
  });
});
```
